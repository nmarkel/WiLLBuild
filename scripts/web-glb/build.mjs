...
async function main() {
  ...
  const wanted = args.parts
    ? population.filter((p) => args.parts.includes(p.id))
    : population.filter((p) => !(p.id in holds))
  if (args.parts) {
    for (const id of args.parts) {
      if (!wanted.some((p) => p.id === id)) {
        throw new Error(`--parts ${id}: not a real-CAD fixture/arm in the catalog`)
      }
    }
  }
  ...
  // Registry hygiene: only current-population, non-held parts stay registered.
  for (const id of Object.keys(registry.models)) {
    if (id in holds || !population.some((p) => p.id === id)) {
