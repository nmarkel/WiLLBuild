...
describe('bannerGeometry (Workstream C)', () => {
  it('derives the panel height and both bar heights from catalog geometry', () => {
    ...
    // BA1 geometry (current catalog): a 1.25 m panel hung between bars whose
    // centres sit symmetrically at ±0.64 m of the mount point.
    expect(geom.panelHeightM).toBeCloseTo(1.25, 3)
    expect(geom.topBarM).toBeCloseTo(8 * 0.3048 + 0.64, 3)
    expect(geom.bottomBarM).toBeCloseTo(8 * 0.3048 - 0.64, 3)
    expect(geom.topBarM - geom.bottomBarM).toBeCloseTo(1.28, 3)
  })
