...
describe('web-GLB IP gate (0.15 B)', () => {
  ...
  it('lists only real-CAD, non-pseudo parts that exist in the catalog', () => {
    // Phase 0.17: the registry outgrew 0.15's live-focus scope — the
    // geometry-service's shell-accurate IFC consumes shells for base covers,
    // poles and accessories too. Any real-CAD, non-pseudo part may register;
    // the LIVE_FOCUS_SLOTS restriction still applies where it always did (the
    // viewer's webModelFor is only ever called for focus slots), and every
    // entry stays hash-bound to the shell report regardless of slot.
