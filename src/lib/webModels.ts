 ...
 * exists". Since Workstream B the registry is GENERATED by
 * `scripts/web-glb/build.mjs` — exterior-shell-only extraction + decimation —
 * and pinned by the IP gate in `webModels.test.ts`: shell re-check
 * (area-weighted visible fraction), lossiness ceiling, byte budget, content
 * hashes binding registry ↔ shell report ↔ file on disk, and a full partition
 * (every real-CAD fixture/arm registered or held with a written reason).
 *
...
export interface WebModelEntry {
  ...
  rotateYDeg?: number
  /**
   * Workstream B: content hash binding this entry to the file the shell gate
   * measured (scripts/web-glb/shell-report.json) — a swapped or stale GLB
   * fails the gate even if its byte count happens to match.
   */
  sha256?: string
}
