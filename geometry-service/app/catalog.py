...
def validate_config(catalog: dict, cfg: PoleConfig) -> None:
    ...
    # (Phase 0.10 Workstream A/D).  Mirrors allowedArmCounts in src/lib/compat.ts:
    # the intersection of the pole's and the arm's `arrangements` (absent → single
    # only).  The ordering matrix owns those lists, so this rejects configs with
    # no design code — e.g. a 3-arm SH1 shepherds hook, which is single-only. ---
